...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    a = abs(get_lat(city_a) - get_lat(city_b))
    b = abs(get_lon(city_a) - get_lon(city_b))
    return sqrt(a * a + b * b)
...
def sprout_leaves(t, leaves):
    ...
    return tree(label(t), [sprout_leaves(subt, leaves) for subt in branches(t)])
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[i//2] if i % 2 == 0 else deck[(len(deck) + i - 1) // 2] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    for i in range(max(len(b1), len(b2))):
        if i >= len(b1) or i >= len(b2):
            b.append(b1[i] if i >= len(b2) else b2[i])
        else:
