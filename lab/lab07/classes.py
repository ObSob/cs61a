...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        assert len(opponent.deck.cards) >= 3
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        # add self's deck attribute
        for c in player.deck.cards:
            c.attack += other_card.attack
            c.defense += other_card.defense

        # remove oppenent deck
        i = 0
        while i < len(opponent.deck.cards):
            c = opponent.deck.cards[i]
            if c.attack == other_card.attack or c.defense == other_card.defense:
