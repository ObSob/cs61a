...
def prune_min(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if not t or not t.branches:
        return
    minb = min(b.label for b in t.branches)
    for b in t.branches:
        if b.label == minb:
            t.branches = [b]
            prune_min(b)
...
def num_splits(s, d):
    ...
    def dfs(ls, suml=0, sumr=0, index=0):
        if index == len(ls):
            return 1 if abs(suml - sumr) <= d else 0
        return dfs(ls, suml + ls[index], sumr, index + 1) + dfs(ls, suml, sumr + ls[index], index + 1)
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        ...
        print("DEBUG:", self.holder, check.payable_to)
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits, costs = helper_align(skeleton_idx + 1, code_idx + 1)
            edits = skel_char + edits
            possibilities.append((edits, costs))
        # Insert
        edits, costs = helper_align(skeleton_idx, code_idx + 1)
        edits = f'+[{code_char}]' + edits
        possibilities.append((edits, costs + 1))
        # Delete
        edits, costs = helper_align(skeleton_idx + 1, code_idx)
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda z, first: Link(first, z), Link.empty)
