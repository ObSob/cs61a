...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = expr.rest.map(lambda a: scheme_eval(a, env))

        # print("DEBUG:", op, operands)
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 7
    val = expressions
    # print('DEBUG', f'{val, type(val)}')
    if val is nil:
        return None
    while val.rest is not nil:
        scheme_eval(val.first, env)
        val = val.rest
    return scheme_eval(val.first, env, True)  # replace this with lines of your own code
    # END PROBLEM 7
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        frame = self
        while frame:
            # print('DEBUG:', frame.bindings.keys(), frame.bindings.get(symbol))
    ...
    def make_child_frame(self, formals, vals):
        ...
        frame = Frame(self)
        k, v = formals, vals
        while k is not nil and v is not nil:
            frame.bindings[k.first] = v.first
            k, v = k.rest, v.rest
        return frame
        # END PROBLEM 10
...
# (map (lambda (formal) body) iterable)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        formals = target.rest
        fn_body = expressions.rest
        procedure = do_lambda_form(Pair(formals, fn_body), env)
        env.bindings[fn_name] = procedure
        # print('DEBUG:', f'{fn_name, procedure}')
        return fn_name
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # print('DEBUG:', f'{formals, expressions.rest}')
...
def do_and_form(expressions, env):
    ...
    while exp.rest is not nil:
        if not is_true_primitive(scheme_eval(exp.first, env)):
            return False
        exp = exp.rest
...
def do_or_form(expressions, env):
    ...
    exp = expressions
    while exp.rest is not nil:
        if is_true_primitive(scheme_eval(exp.first, env)):
            return scheme_eval(exp.first, env, True)
        exp = exp.rest
    return scheme_eval(exp.first, env, True)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bind is not nil:
        # print("DEBUG:", f'{bind.first, bind.first.first, bind.first.rest}')
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        # print("DEBUG:", f'{name, formals, body}')
        mp = MacroProcedure(formals, body, env)
        env.define(name, mp)
        return name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            result = original_scheme_eval(result.expr, result.env, True)
        return result
        # END PROBLEM 19
