class VendingMachine:
    ...
    def restock(self, num):
        self.remain += num
        return 'Current {} stock: {}'.format(self.name, self.remain)

    def add_funds(self, fond):
        if self.remain == 0:
            return 'Inventory empty. Restocking required. Here is your ${}.'.format(fond)
        else:
            self.balance += fond
    ...
    def vend(self):
        ...
        if self.balance >= self.price:
            coins = self.balance - self.price
            self.balance -= 0
            self.remain -= 1
            if coins > 0:
                self.balance = 0
                return 'Here is your {} and ${} change.'.format(self.name, coins)
            else:
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        his = Mint.current_year - self.year
        if his > 50:
            return self.cents + his - 50
        else:
            return self.cents
...
def store_digits(n):
    ...
    if n < 10:
        ...
        while p * 10 < n:
            p *= 10
        return Link(n // p, store_digits(n % p))
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [t.label]
    for b in t.branches:
        for it in path_yielder(b, value):
            "*** YOUR CODE HERE ***"
            it.insert(0, t.label)
            yield it
